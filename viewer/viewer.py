...
def from_json(obj):
    ...
    ntwk.z0 = (
        np.array(obj["_z0"])[..., 0] + np.array(obj["_z0"])[..., 1] * 1j
...
@app.callback(
    [Output("port-table-div", "open"), Output("port-table", "data")],
...
@app.callback(
    [Output("uploaded-data-table", "data"), Output("data-uploading", "children")],
...
@app.callback(
    ...
    [State("graph1", "figure"), State("graph2", "figure")],
)
        ...
        return html.Div(
            html.Details(
                [
                    ...
                    html.Button(
                        id="timegate-button", children="Time Gate Visible Traces"
                    ),
...
for slider in ["freq-gate-slider", "time-gate-slider"]:
    ...
    @app.callback(Output(f"{slider}-value", "children"), [Input(slider, "value")])
...
def time_gate_plot(_, fig1, fig2, fgate, tgate):
    ...
    for d in fig1["data"]:
        ...
        ftracelist.append(
            go.Scatter(
                x=ntwk[f"{fgate[0]}-{fgate[1]}"].f,
                y=ntwk[f"{fgate[0]}-{fgate[1]}"]
                .time_gate(start=tgate[0] * 1e9, stop=tgate[1] * 1e9)
                .s_mag.flatten(),
                mode="lines",
        ...
        ttracelist.append(
            go.Scatter(
                x=ntwk[f"{fgate[0]}-{fgate[1]}"].frequency.t,
                y=ntwk[f"{fgate[0]}-{fgate[1]}"]
                .time_gate(start=tgate[0] * 1e9, stop=tgate[1] * 1e9)
                .s_time_mag.flatten(),
                mode="lines",
...
def update_graph(
    ...
    if not selected_ntwk_rows:
                ...
                children=[
                    ...
                    html.Div(
                        children=[
                            ...
                            "Jackson Anderson,",
                            html.A(
                                "ander906@purdue.edu", href="mailto:ander906@purdue.edu"
                            ),
                        ],
                    ),
                ],
        ...
        return (
            html.Div(children="Please Select Ports to Plot."),
            dcc.Graph(id="graph1", figure={"data": []}, style={"display": "none"}),
            dcc.Graph(id="graph2", figure={"data": []}, style={"display": "none"}),
            plotted_axes_format_output,
        )
        ...
        try:
            ...
            return (
                html.Div(
                    children="Data could not be retrieved from cache. "
        ...
        if axes_format == "MAG":
            layout1.append(
                go.Layout(
                    xaxis={"title": "Frequency [Hz]", "exponentformat": "SI"},
                    yaxis={
                        "type": "log",
            ...
            layout1.append(
                go.Layout(
                    xaxis={"title": "Frequency [Hz]", "exponentformat": "SI"},
                    yaxis={"type": "linear", "title": "{} Parameter Real".format(parm)},
                    margin={"l": 60, "b": 40, "t": 10, "r": 10},
                    legend={"x": 1, "y": 1},
                    hovermode="closest",
                )
            ...
            layout2.append(
                go.Layout(
                    xaxis={"title": "Time [s]", "exponentformat": "SI"},
                    yaxis={
                        "type": "log",
